...
impl Room {
    ...
    pub fn restart(&self, active_player_ids: &[u64]) {
        let mut state = self.state.lock().expect("room state mutex poisoned");
        let previous_input_seqs = state
            .players
            .iter()
            .map(|(&player_id, player)| (player_id, player.last_received_input_seq))
            .collect::<HashMap<_, _>>();
        ...
        for &player_id in active_player_ids {
            state.players.insert(
                ...
                Player {
                    last_input: PlayerInput::default(),
                    last_received_input_seq: previous_input_seqs
                        .get(&player_id)
                        .copied()
                        .unwrap_or(0),
                },
    ...
    pub fn update_input(&self, player_id: u64, input_seq: u64, input: PlayerInput) {
        let mut state = self.state.lock().expect("room state mutex poisoned");
        let Some(player) = state.players.get_mut(&player_id) else {
            return;
        };

        if input_seq <= player.last_received_input_seq {
            return;
        }

        player.last_received_input_seq = input_seq;
    ...
    pub fn tick(&self, delta_seconds: f32) {
        ...
        let inputs = state
            .players
            .iter_mut()
            .map(|(&player_id, player)| (player_id, player.last_input))
            .collect::<Vec<_>>();
    ...
    pub fn snapshot_for_player(&self, player_id: u64) -> RoomSnapshot {
        ...
        RoomSnapshot {
            ...
            last_received_input_seq: state
                .players
                .get(&player_id)
                .map_or(0, |player| player.last_received_input_seq),
        }
    }
...
mod tests {
    ...
    #[test]
    fn missing_input_reuses_last_input() {
        let room = Room::new(test_config());
        room.add_player(1);
        room.update_input(
            1,
    ...
    #[test]
    fn duplicate_and_out_of_order_inputs_are_ignored() {
        let room = Room::new(test_config());
        room.add_player(1);
        room.update_input(
            1,
    ...
    #[test]
    fn restart_preserves_last_received_input_seq() {
        let room = Room::new(test_config());
        room.add_player(1);
        room.update_input(
            1,
        ...
        room.restart(&[1]);
        assert_eq!(room.snapshot_for_player(1).last_received_input_seq, 5);
        ...
        let tick_two = player_snapshot(&room);
        assert!(tick_two.vx > 0.0);
        assert_eq!(room.snapshot_for_player(1).last_received_input_seq, 6);
    }
}
